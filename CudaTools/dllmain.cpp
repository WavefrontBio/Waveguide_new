	...
	float xmin, float xmax, float ymin, float ymax,	int maxNumDataPoints, int numTraces, CudaChartArray** pp_chartArray)
{
	...
	float2 xRange = { xmin,xmax };
	float2 yRange = { ymin,ymax };
...
DllExport void AppendData(CudaChartArray* pChartArray, float* xArray, float* yArray, int numPoints, int traceNum)
{	
	float2* newPoints = (float2*)malloc(numPoints * sizeof(float2));
...
DllExport void* GetRangePtr(CudaChartArray* pChartArray, int traceNum)
{
	range[0] = pChartArray->m_x_min[traceNum];
	range[1] = pChartArray->m_x_max[traceNum];
	range[2] = pChartArray->m_y_min[traceNum];
	range[3] = pChartArray->m_y_max[traceNum];
...
DllExport void SetInitialRanges(CudaChartArray* pChartArray, int traceNum, float xmin, float xmax, float ymin, float ymax)
{
	pChartArray->SetInitialRanges(traceNum, xmin, xmax, ymin, ymax);
}
