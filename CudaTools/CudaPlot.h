...
public:
	...
	void AppendData(float2 *p_new_points, int traceNum);
	...
	void SetInitialRanges(int traceNum, float xmin, float xmax, float ymin, float ymax);
	...
	float2* mp_d_data[MAX_TRACES];  // device pointer to data block
	...
	float  m_x_min[MAX_TRACES];
	float  m_x_max[MAX_TRACES];
	float  m_y_min[MAX_TRACES];
	float  m_y_max[MAX_TRACES];
	...
	float  m_x_value_to_pixel[MAX_TRACES];
	float  m_y_value_to_pixel[MAX_TRACES];
	float  m_x_value_to_pixel_aggregate[MAX_TRACES];
	float  m_y_value_to_pixel_aggregate[MAX_TRACES];

	float2   m_initial_xRange[MAX_TRACES];
	float2   m_initial_yRange[MAX_TRACES];
};
