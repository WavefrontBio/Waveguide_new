...
CudaChartArray::CudaChartArray(int rows, int cols, 
	...
	float2 xRange, float2 yRange, int maxNumDataPoints, int numTraces)
{
	...
	{
		m_x_min[i] = xRange.x;
		m_x_max[i] = xRange.y;  if (m_x_max[i] == m_x_min[i]) m_x_max[i]++; // make sure that there is not a zero range
		m_y_min[i] = yRange.x;
		m_y_max[i] = yRange.y;  if (m_y_max[i] == m_y_min[i]) m_y_max[i]++; // make sure that there is not a zero range

		m_initial_xRange[i].x = m_x_min[i];
		m_initial_xRange[i].y = m_x_max[i];
		m_initial_yRange[i].x = m_y_min[i];
		m_initial_yRange[i].y = m_y_max[i];
	}
...
__global__ void plotChart(float2* data, uchar4* output, int numPoints, int maxNumPoints, float convX, float convY,
	int panelWidth, int panelHeight, size_t windowPitch,
	float plotMinX, float plotMaxX, float plotMinY, float plotMaxY,
...
__global__ void plotAggregateChart(float2* data, uchar4* output, int numPoints, int maxNumPoints, float convX, float convY,
	...
	uchar4 plotColor, int margin, bool* selected)
{
	...
	//	float2 pt = mp_d_data[(dataRowLength * n) + (r * m_cols) + c];
...

{
	// each group of data points make up one row of data
	//
	//  int dataRowLength = numChartRows * numChartCols * sizeof(float2);
	//
	// So, to get the data point (pt) for row = r, col = c, and data point number = n
	//
	//	float2 pt = mp_d_data[(dataRowLength * n) + (r * m_cols) + c];
	...
	size_t blocksize = m_rows * m_cols * sizeof(float2);  // this is the size of one group of data (one point for each chart)
...
void CudaChartArray::CalcConversionFactors()
{
	...
	for (int i = 0; i < m_numTraces; i++)
	{
		m_x_value_to_pixel[i] = (float)(m_chart_width) / (float)(m_x_max[i] - m_x_min[i]);
		m_y_value_to_pixel[i] = (float)(m_chart_height) / (float)(m_y_max[i] - m_y_min[i]);

		m_x_value_to_pixel_aggregate[i] = (float)(m_aggregate_width - (2 * m_margin)) / (float)(m_x_max[i] - m_x_min[i]);
		m_y_value_to_pixel_aggregate[i] = (float)(m_aggregate_height - (2 * m_margin)) / (float)(m_y_max[i] - m_y_min[i]);
	}
}

void CudaChartArray::AppendData(float2 *p_new_points, int traceNum)
{
	bool rangeChanged = false;

	if (traceNum > m_numTraces - 1 || traceNum < 0) return;

	uint8_t* ptr = (uint8_t*)mp_d_data[traceNum] + (m_num_data_points[traceNum] * m_rows * m_cols * sizeof(float2));


	cudaError_t result = cudaMemcpy(ptr, (void*)p_new_points, m_rows * m_cols * sizeof(float2), cudaMemcpyHostToDevice);

	if (result == CUDA_SUCCESS)
	{
		for (int i = 0; i < m_rows * m_cols; i++)
		{
			if (p_new_points[i].x > m_x_max[traceNum]) { m_x_max[traceNum] = p_new_points[i].x * 11 / 10; rangeChanged = true; }
			if (p_new_points[i].y > m_y_max[traceNum]) { m_y_max[traceNum] = p_new_points[i].y * 6 / 5;	rangeChanged = true; }
			if (p_new_points[i].x < m_x_min[traceNum]) { m_x_min[traceNum] = p_new_points[i].x;	rangeChanged = true; }
			if (p_new_points[i].y < m_y_min[traceNum]) { m_y_min[traceNum] = p_new_points[i].y; rangeChanged = true; }			
		}

		if (m_x_max[traceNum] == m_x_min[traceNum]) m_x_max[traceNum]++; // make sure that there is not a zero range
		if (m_y_max[traceNum] == m_y_min[traceNum]) m_y_max[traceNum]++; // make sure that there is not a zero range
...
void CudaChartArray::Redraw()
{
	...
	drawEmptyCharts << <numBlocks, threadsPerBlock >> > (mp_d_chart_image, m_chartArray_width, m_chartArray_height, m_chart_image_pitch,
		m_rows, m_cols, m_margin, m_padding, m_chart_width, m_chart_height, m_y_min[0], m_y_max[0],
	...
	for (int i = 0; i < m_numTraces; i++)
	{
		if(m_traceVisible[i])
			plotChart << <numBlocks1, threadsPerBlock1 >> > (mp_d_data[i], mp_d_chart_image, m_num_data_points[i], 
				m_max_num_data_points, m_x_value_to_pixel[i], m_y_value_to_pixel[i],
				m_chart_width, m_chart_height, m_chart_image_pitch,
				m_x_min[i], m_x_max[i], m_y_min[i], m_y_max[i],
...
void CudaChartArray::SetInitialRanges(int traceNum, float xmin, float xmax, float ymin, float ymax)
{
	m_initial_xRange[traceNum].x = xmin;
	m_initial_xRange[traceNum].y = xmax;
	m_initial_yRange[traceNum].x = ymin;
	m_initial_yRange[traceNum].y = ymax;
}
...
void CudaChartArray::RedrawAggregate()
{
	...
	drawEmptyAggregateChart << <numBlocks, threadsPerBlock >> > (mp_d_aggregate_image, m_aggregate_width, m_aggregate_height, m_aggregate_image_pitch,
		m_rows, m_cols, m_margin, m_y_min[0], m_y_max[0],
	...
	for (int i = 0; i < m_numTraces; i++)
	{
		if (m_traceVisible[i])
			plotAggregateChart << <numBlocks1, threadsPerBlock1 >> > (mp_d_data[i], mp_d_aggregate_image, 
				...
				m_x_value_to_pixel_aggregate[i], m_y_value_to_pixel_aggregate[i],
...
void CudaChartArray::Reset()
{
	for (int i = 0; i < m_numTraces; i++)
	{
		...
		m_x_min[i] = m_initial_xRange[i].x;
		m_x_max[i] = m_initial_xRange[i].y;
		m_y_min[i] = m_initial_yRange[i].x;
		m_y_max[i] = m_initial_yRange[i].y;

		if (m_x_max[i] == m_x_min[i]) m_x_max[i]++; // make sure that there is not a zero range
		if (m_y_max[i] == m_y_min[i]) m_y_max[i]++; // make sure that there is not a zero range
	}
